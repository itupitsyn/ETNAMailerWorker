...
async fn process_mailer(
    ...
    let rows = sqlx::query(r#"SELECT id, "phoneNumber", email, name, theme, message, "createdAt" FROM "Request" WHERE "isSent" = false"#)
        .fetch_all(pool)
        .await?;
    ...
    for row in rows {
        ...
        let phone: String = sqlx::Row::get(&row, "phoneNumber");
        let email: String = sqlx::Row::get(&row, "email");
        let theme: String = sqlx::Row::get(&row, "theme");
        let message: String = sqlx::Row::get(&row, "message");
...
<body style=\"margin: 0; padding: 0\">
  ...
  <table style=\"border-collapse: collapse;\">
    ...
    let subject = format!("[landing] Новые заявки: {} шт", req_count);
